...
// --service=slug[,slug]: generate ONLY these services and merge them into each
// town's existing block. Without it, a town run regenerates every service.
const ONLY_SERVICES = getArg("service")?.split(",").map((s) => s.trim()).filter(Boolean);

const SERVICES = serviceDetails
  .filter((s: { slug: string }) => !ONLY_SERVICES || ONLY_SERVICES.includes(s.slug))
  .map((s: { slug: string; title: string; subtitle?: string; features?: string[] }) => ({
    slug: s.slug, title: s.title, subtitle: s.subtitle ?? "", features: s.features ?? [],
  }));
if (ONLY_SERVICES && SERVICES.length !== ONLY_SERVICES.length) {
  throw new Error(`--service names unknown slug(s): ${ONLY_SERVICES.filter((x) => !SERVICES.some((s) => s.slug === x)).join(", ")}`);
}
...
${SERVICES.map((s) => `- "${s.slug}" (${s.title}): ${s.subtitle}${s.features.length ? ` | Scope: ${s.features.join(", ")}` : ""}`).join("\n")}
...
async function main() {
  ...
  // Merge PER SERVICE inside each town, never by whole-town replacement — a
  // --service run must not discard the other services' existing baked copy.
  const merged: Record<string, Record<string, string>> = {};
  for (const k of Object.keys({ ...existing, ...result }).sort()) {
    merged[k] = { ...(existing[k] ?? {}), ...(result[k] ?? {}) };
  }
