  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      <div className="mx-auto max-w-7xl px-6 md:px-8 py-5">
        ...
        {/* Mobile menu */}
        {mobileOpen && (
          <div className="md:hidden mt-6 pb-6 border-t border-white/10 pt-6 flex flex-col gap-4">
            {links.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                ...
                className={`text-lg font-display ${
                  pathname === link.href ? "text-cream" : "text-stone-dark/60"
                }`}
              >
                {link.label}
              </Link>
            ...
            <a
              href="tel:802-342-8293"
              className="mt-2 flex items-center gap-2 text-sm text-copper-light"
            >
