...
export function AboutPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(".serve-card", { opacity: 0, y: 20 }, {
        opacity: 1, y: 0, duration: 0.5, stagger: 0.08, ease: "power2.out",
        scrollTrigger: { trigger: ".serve-grid", start: "top 80%" },
      });

      gsap.fromTo(".town-tag", { opacity: 0, scale: 0.9 }, {
    ...
    <InnerLayout>
      ...
      <section className="py-24 md:py-32 bg-soil-light/30">
        <div className="mx-auto max-w-7xl px-6 md:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <div className="fade-up">
              <p className="text-xs font-medium uppercase tracking-[0.3em] text-copper-light">Technology & Accountability</p>
              <h2 className="mt-4 font-display text-3xl md:text-4xl tracking-tight leading-tight">
                Professional systems.<br /><span className="text-stone italic">Real-time accountability.</span>
              </h2>
              <p className="mt-6 text-[15px] leading-relaxed text-stone-dark/60">
                We use a CSM management system, employee portal, and GPS-tracked service vehicles to help ensure a higher standard of service across our operations.
              </p>
              <ul className="mt-8 space-y-4">
                {techPoints.map((point) => (
                  <li key={point} className="flex items-center gap-3 text-sm text-stone-dark/70">
                    <Satellite className="h-4 w-4 text-forest-light flex-shrink-0" strokeWidth={1.5} />
                    {point}
                  </li>
                ))}
              </ul>
            ...
            <div className="fade-up relative rounded-2xl overflow-hidden aspect-[4/5]">
              <img src="/images/bg-property-maintenance.jpeg" alt="Meticulous property maintenance" className="w-full h-full object-cover" />
              <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-soil/80 to-transparent">
                <p className="font-display text-lg text-cream italic">Professional systems. Real-time accountability.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 md:py-32">
        <div className="mx-auto max-w-7xl px-6 md:px-8">
          <div className="fade-up text-center max-w-2xl mx-auto mb-16">
            <p className="text-xs font-medium uppercase tracking-[0.3em] text-copper-light">Who We Serve</p>
            <h2 className="mt-4 font-display text-3xl md:text-4xl tracking-tight">
              Property care built for<br /><span className="text-stone italic">the way you use your property.</span>
            </h2>
            <p className="mt-6 text-[15px] leading-relaxed text-stone-dark/60">
              Whether your property is your home, an investment, or part of your business, we&apos;re here to help keep it maintained, protected, and presentation-ready.
            </p>
          </div>
          <div className="serve-grid grid grid-cols-2 md:grid-cols-3 gap-6">
            {serveBlocks.map((s) => (
              <div key={s.label} className="serve-card flex items-center gap-4 p-6 rounded-2xl bg-soil-light/20 ring-1 ring-white/5">
                <s.icon className="h-5 w-5 text-forest-light flex-shrink-0" strokeWidth={1.5} />
      ...
      <section className="py-24 md:py-32 bg-soil-light/30">
        <div className="mx-auto max-w-4xl px-6 md:px-8 text-center">
          <div className="fade-up">
            <p className="text-xs font-medium uppercase tracking-[0.3em] text-copper-light">Service Area</p>
            <h2 className="mt-4 font-display text-3xl md:text-4xl tracking-tight">
              Proudly serving<br /><span className="text-stone italic">Vermont properties.</span>
            </h2>
            <p className="mt-6 text-[15px] leading-relaxed text-stone-dark/60 max-w-2xl mx-auto">
              Meticulous LLC proudly serves clients throughout Rutland County and surrounding Vermont communities.
            </p>
          </div>
          <div className="towns-grid mt-12 flex flex-wrap justify-center gap-3">
            {towns.map((town) => (
              <span key={town} className="town-tag inline-flex items-center gap-2 rounded-full bg-soil px-5 py-2.5 text-sm font-medium text-cream ring-1 ring-white/10">
          ...
          <p className="mt-8 text-sm text-stone-dark/50">
            Not sure if your property falls within our service area? Feel free to reach out.
          </p>
        </div>
      </section>
      ...
      <section className="py-24 md:py-32">
        <div className="mx-auto max-w-3xl px-6 md:px-8 text-center">
          <div className="fade-up">
            <p className="text-xs font-medium uppercase tracking-[0.3em] text-copper-light">Community</p>
            <h2 className="mt-4 font-display text-3xl md:text-4xl tracking-tight">
              Rooted in<br /><span className="text-stone italic">the community.</span>
            </h2>
            <p className="mt-6 text-[15px] leading-relaxed text-stone-dark/60 max-w-xl mx-auto">
              We believe in doing more than just working in the communities we serve — we believe in being part of them. Supporting local families, organizations, and community initiatives is something we value and take pride in.
            </p>
          </div>
        </div>
      </section>
